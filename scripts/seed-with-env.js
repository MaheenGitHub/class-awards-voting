...
async function seedDatabase() {
  try {
    ...
    console.log(`👤 Admin email set to: ${process.env.NEXT_PUBLIC_ADMIN_EMAIL}`);

    // Initialize Firebase
    const app = initializeApp(firebaseConfig);
    const db = getFirestore(app);

    // Clear categories collection first to remove duplicates
    console.log('🗑️  Clearing categories collection to remove duplicates...');
    ...
    // Add categories (using deterministic IDs)
    ...
    console.log(`👤 Admin access configured for: ${process.env.NEXT_PUBLIC_ADMIN_EMAIL}`);
    ...
    if (finalStudents.size === 40) {
      console.log('✅ Perfect! Exactly 40 students as required.');
    } else {
      console.log(`⚠️  Expected 40 students, found ${finalStudents.size}`);
    }
