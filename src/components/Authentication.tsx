'use client'

import { useAuth } from '@/contexts/AuthContext'
import VotingInterface from './VotingInterface'

export default function Authentication() {
  const { user, signIn, isLoading } = useAuth()

  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-2 border-gray-300 border-t-blue-600 mx-auto mb-4"></div>
          <p className="title-text">Loading...</p>
        </div>
      </div>
    )
  }

  if (!user) {
    return (
      <div className="min-h-screen flex items-center justify-center px-4 py-12">
        <div className="max-w-md w-full">
          <div className="professional-card p-8 fade-in">
            {/* Header */}
            <div className="text-center mb-8">
              <h1 className="title-text text-2xl mb-4">Class Voting Awards</h1>
              <p className="description-text">
                Vote for your classmates in various award categories
              </p>
            </div>

            {/* Anonymity Notice */}
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
              <div className="flex items-center mb-2">
                <svg className="w-5 h-5 text-blue-600 mr-2" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clipRule="evenodd" />
                </svg>
                <h3 className="title-text text-sm">Your vote is anonymous</h3>
              </div>
              <p className="small-text text-blue-700">
                Your email is only used once to verify you belong to the class and is not stored with your votes.
              </p>
            </div>

            {/* Important Notice */}
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6">
              <p className="description-text">
                <span className="font-medium">Important:</span> Only students with 
                <span className="inline-block bg-white border border-gray-300 px-2 py-1 rounded ml-1 font-mono text-sm">
                  bitf22mXXX@pucit.edu.pk
                </span>
                {" "}email addresses can vote.
              </p>
            </div>

            {/* Google Sign-in Button */}
            <button
              onClick={signIn}
              className="professional-button w-full"
            >
              <svg className="w-5 h-5" viewBox="0 0 24 24">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
              </svg>
              Sign in with Google
            </button>

            {/* Footer */}
            <div className="text-center mt-6">
              <p className="small-text">
                By signing in, you confirm you are a student of this class and agree to vote only once.
              </p>
            </div>
          </div>
        </div>
      </div>
    )
  }

  return <VotingInterface />
}
